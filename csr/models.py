...
class GasUtilServiceRequest(models.Model):
    ...
    created_on = models.DateTimeField(auto_now=True)
    resolved_on = models.DateTimeField(default=timezone.now, blank=True, null=True)  # Use timezone.now without parentheses
    service_paid_on = models.DateTimeField(default=timezone.now, blank=True, null=True)  # Use timezone.now without parentheses
